...
@Component({
  selector: 'app-alquilados-list',
  templateUrl: './alquilados-list.component.html',
  styles: [
    `:host {
      padding: 10px;
    }`
...
export class AlquiladosListComponent implements OnInit {
  ...
  selectedItem: IEquipoAlquiladoValores;
  page = new Page();
  loaded = false;
  ...
  refresh(newPage?) {
    ...
    this.loaded = false;
    this.tallerServ.get_alquilados_valores(
      this.page.pageNumber,
      this.f_valido_desde,
      this.f_equipo
    ).subscribe(
      valores => {
        this.valores = valores['results'] as Array<IEquipoAlquiladoValores>;
  ...
  selectItem(item) {
    ...
    this.router.navigate(['/taller/valores', {
      outlets: {'details': ['alquilados', this.selectedItem.pk]}
    }]);
  }
